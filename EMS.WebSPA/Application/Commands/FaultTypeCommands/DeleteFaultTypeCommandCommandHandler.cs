...
namespace EMS.WebSPA.Application.Commands.FaultTypeCommands{
    public class DeleteFaultTypeCommandCommandHandler : IRequestHandler<DeleteFaultTypeCommand, bool>{
        private readonly IEmsQuery _emsQuery;
        private readonly IFaultTypeRepository _faultTypeRepository;
        public DeleteFaultTypeCommandCommandHandler(IEmsQuery emsQuery, IFaultTypeRepository faultTypeRepository){
            _emsQuery = emsQuery;
            _faultTypeRepository = faultTypeRepository;
        }
        public async Task<bool> Handle(DeleteFaultTypeCommand request, CancellationToken cancellationToken){
            if (await _emsQuery.IsExistItemWithFaultType(request.Id))
                throw new EMSDomainException("با این نوع، آیتم ثبت شده است ، شما قادر به حذف نیستید");
            ...
            _faultTypeRepository.Remove(faultType);
            return await _faultTypeRepository.UnitOfWork.SaveEntitiesAsync(cancellationToken);
        }
    }
    public class DeleteFaultTypeIdentifiedCommandHandler : IdentifierCommandHandler<DeleteFaultTypeCommand, bool>{
        public DeleteFaultTypeIdentifiedCommandHandler(IMediator mediator, IRequestManager requestManager,
